            ...
            renderItem={({ item }) => {
              ...
              return (
                <View style={styles.card}>
                  <Text style={styles.cardText}>Email atual: {item.email}</Text>
                  <Text style={styles.cardText}>Senha atual: {item.senha}</Text>

                  <Text style={{ marginTop: 8 }}>Editar Email:</Text>
                  <TextInput
                    value={editState.email}
                    onChangeText={(text) =>
                      setEditFieldsUsuarios((prev) => ({
                        ...prev,
                        [item.id]: { ...prev[item.id], email: text },
                      }))
                    }
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        atualizarItem(
                          ...
                          {
                            email:
                              editFieldsUsuarios[item.id]?.email ?? item.email,
                            senha:
